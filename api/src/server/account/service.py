...
class AccountService(object):
    ...
    def register(data):
        ...
        account = AccountModel(
            email=data["email"],
            password=AccountAuth.hash_password(data["password"])
        )

        # Check for duplicate username & email
        # TODO: Replace with guards
        if AccountDao.find_by_email(account.email): raise DuplicateEmailException
    ...
    def login(data):
        ...
        if account is None: raise AccountNotFoundException
        
        print("Account password", account.password)
        print("\n\n")
        
        # Check password match
        if not AccountAuth.is_matching_password(account.password, data["password"]): raise InvalidCredentialsException
    ...
    def get_account_by_token(data):
        try:
            ...
            if account is None: raise InvalidCredentialsException
            
            return account
            
        # Catch jwt.ExpiredSignatureError & jwt.InvalidTokenError
        except jwt.ExpiredSignatureError: raise InvalidCredentialsException
        except jwt.InvalidTokenError: raise InvalidCredentialsException
...
#   - When client makes request to /refresh, a new access_token is sent and client will store in memory & make request again
# If client makes request to secure endpoint that requires login, then return LoginRequiredMessage, regardless of refresh token
