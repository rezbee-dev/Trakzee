...
class AccountAuth(object):
    ...
    @staticmethod
    def encode_token(account_id, token_type):
        seconds = (
            current_app.config.get("ACCESS_TOKEN_EXPIRATION")
            if token_type == "access"
            else current_app.config.get("REFRESH_TOKEN_EXPIRATION")
        )
        ...
        return jwt.encode(payload=jwt_payload, key=current_app.config.get("SECRET_KEY"), algorithm="HS256")
